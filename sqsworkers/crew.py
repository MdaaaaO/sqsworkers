...
class Crew():
    ...
    def __init__(self, **kwargs):
        ...
        self.logger = logging.LoggerAdapter(
            kwargs['logger'], extra={'extra': {'crew.name': self.name}})
        self.statsd = kwargs['statsd'] if 'statsd' in kwargs else DummyStatsd(
            self.logger)
        ...
        if not ((self.sqs_session and self.queue_name) or self.sqs_resource):
            raise TypeError(
                'Required arguments not provided.  Either provide (sqs_session + queue_name) or sqs_resource.')
...
class Worker(CrewMember):
    def __init__(self, crew):
        ...
        self.worker_name = 'worker-%s-%s-%s' % (
            os.getpid(), currentThread().getName(), str(time.time()))
        ...
        self.crew.logger.info(
            'new worker starting with name: %s' % (self.worker_name))
        self.logger = logging.LoggerAdapter(self.crew.logger, extra={
                                            'extra': {'worker_name': self.worker_name, 'crew.name': self.crew.name}})
    ...
    def _wrap_run(self):
        try:
            ...
            log_uncaught_exception(e, logger=self.logger, context={
                                   'worker_name': self.worker_name, 'crew.name': self.crew.name})

    def run(self):
        self.logger.info('thread %s starting now' % self.worker_name)
        if self.sqs_resource == None:
            sqs_connection = self.sqs_session.resource(
                'sqs', region_name=self.sqs_session.region_name)
            self.queue = sqs_connection.get_queue_by_name(
        ...
        while self.employed:
            ...
            if len(messages) > 0:
                for message in messages:
                    try:
                        self.logger.info('processing %s messages %s' %
                                         (len(messages), messages))
                        processor = self.crew.MessageProcessor(message)
                        self.crew.statsd.increment(
                            'process.record.start', 1, tags=[])
...
class Supervisor(CrewMember):
    def __init__(self, crew):
        self._real_run = self.run
        self.run = self._wrap_run
        self.supervisor_name = 'supervisor-%s-%s' % (
    ...
    def _wrap_run(self):
        try:
            ...
            self.crew.sentry.captureException() if self.crew.sentry else None
            log_uncaught_exception(e, logger=self.crew.logger, context={
                                   'supervisor_name': self.supervisor_name, 'crew.name': self.crew.name})
            self._wrap_run()
    ...
    def supervise(self):
        good_workers = []
        self.crew.logger.info('supervising %s worker on workers: %s' % (
            self.supervisor_name, self.crew.workers))
        for worker in self.crew.workers:
            if worker.is_alive():
                self.crew.logger.info('worker %s is alive' %
                                      (worker.worker_name))
                good_workers.append(worker)
            else:
                self.crew.logger.warn(
                    'worker %s is dead, hiring a new one...' % (worker.worker_name))
                self.crew.statsd.increment(
                    'workers.dead', 1, tags=[self.crew.name])
                new_worker = Worker(self.crew)
                self.crew.logger.info(
                    'hired a new worker %s, staring it up...' % (new_worker.worker_name))
                new_worker.start()
                self.crew.logger.info('started a new worker %s' %
                                      (new_worker.worker_name))
                good_workers.append(new_worker)
        self.crew.logger.info(
            'total number of workers after supervision: %s' % (str(len(good_workers))))
        self.crew.statsd.gauge('workers.employed', len(
            good_workers), tags=[self.crew.name])
